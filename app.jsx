    ...
    return (
      <div className={darkMode ? 'dark' : ''}>
        <div className="min-h-screen bg-gray-100 dark:bg-gray-950 flex items-center justify-center p-4 transition-colors duration-300">
          <div className="w-full max-w-md bg-white dark:bg-gray-900 p-8 rounded-3xl shadow-2xl border border-gray-200 dark:border-gray-800">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div className="flex items-center gap-3 bg-gray-50 dark:bg-gray-800 p-3 rounded-2xl border dark:border-gray-700">
                <span className="bg-red-600 text-white px-3 py-1 rounded-lg font-bold">{captcha.question} =</span>
                <input 
                  type="number" 
                  placeholder="?" 
        ...
        <nav className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 sticky top-0 z-50">
          <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={handleLogout} className="p-3 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-2xl transition">
                <LogOut size={20}/>
              </button>
            </div>
          </div>
...
const UploadTab = ({ user }) => {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus({ type: 'error', msg: 'Erro ao processar o upload.' });
      }
    } catch (e) {
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white dark:bg-gray-900 p-8 sm:p-12 rounded-[3rem] shadow-2xl border dark:border-gray-800">
    ...
    <div className="bg-white dark:bg-gray-900 rounded-3xl overflow-hidden shadow-sm border border-gray-200 dark:border-gray-800 group hover:shadow-2xl transition-all duration-500 relative">
      <div className="aspect-square bg-gray-50 dark:bg-gray-950 flex items-center justify-center relative overflow-hidden">
        {isVideo ? (
          ...
          <img src={mediaUrl} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-125" loading="lazy" alt="Produto" />
        )}
...
const Pagination = ({ current, total, onChange }) => (
  <div className="flex justify-center items-center gap-6 bg-white dark:bg-gray-900 p-4 rounded-3xl border dark:border-gray-800 shadow-xl w-fit mx-auto mt-10">
     <button disabled={current === 1} onClick={() => onChange(current - 1)} className="font-bold p-2 disabled:opacity-20">Anterior</button>
     <span className="text-xs font-black uppercase tracking-widest text-gray-400">Página {current} de {total}</span>
     <button disabled={current === total} onClick={() => onChange(current + 1)} className="font-bold p-2 disabled:opacity-20">Próxima</button>
  </div>
);
